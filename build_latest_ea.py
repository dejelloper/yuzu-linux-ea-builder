...
LATEST_DOWNLOADED_VER_TXT_PATH = "last_downloaded_ver.txt"
...
if not os.path.exists(LATEST_DOWNLOADED_VER_TXT_PATH):
    temp = open(LATEST_DOWNLOADED_VER_TXT_PATH, "w")
    temp.write("")
    temp.close()

lastDownloadedVersionFile = open(LATEST_DOWNLOADED_VER_TXT_PATH, "r")
lastDownloaded = lastDownloadedVersionFile.read()
...
if len(lastDownloaded) > 0 and int(lastDownloaded) >= latestVersion:
    ...
    for releaseEntry in listingData["files"]:
        if YUZU_SRC_LZMA_REGEX.match(releaseEntry["name"]):
            ...
            lastDownloadedVersionFile.close()
            os.remove(LATEST_DOWNLOADED_VER_TXT_PATH)
            lastDownloadedVersionFile = open(LATEST_DOWNLOADED_VER_TXT_PATH, "w")
            lastDownloadedVersionFile.write(str(latestVersion))
            lastDownloadedVersionFile.close()
